...
// ** Store & Actions
import {  getData, deleteDimensionLevel, getDimensionLevel, resetUpdateResponse } from './store/action'
...
const dimensionLevelList = ({dimensionId}) => {
  // ** Store Vars
      ...
      cell: row => (
        <UncontrolledDropdown className="">
          ...
          <DropdownMenu right>
            <DropdownItem
              className='w-100'
              onClick={() => updateDimensionLevel(row.levelNumber)}
            >
              <Archive size={14} className='mr-50' />
              <span className='align-middle'><FormattedMessage id="Edit" /></span>
            </DropdownItem>
            <DropdownItem className='w-100'  onClick={() => { confirmDelete(deleteDimensionLevel, row.id) }} >
    ...
    <Fragment>
      { isAuthorized(store.errorCode) ? <Redirect to='/misc/not-authorized' /> : (
        <>
              ...
              subHeaderComponent={
                <div className='w-100'>
                  ...
                  {/* <div className="my-1 d-flex justify-content-end">
                    <Button.Ripple color='primary' onClick={addDimensionLevel} >
          ...
          <Sidebar dimensionId={dimensionId} open={sidebarOpen} toggleSidebar={toggleSidebar} selectedDimensionLevel={store.selectedDimensionLevel} />
        </>
      )}
    </Fragment>
  )
...
export default dimensionLevelList
