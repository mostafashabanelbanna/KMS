...
const SearchForm = (props) => {
  ...
  const formItems = props.formConfig.map((item, index) => {
        switch (item.fieldType) {
            ...
            case 'select':
                return (<Col  key={index} className='my-2' lg="4"> <CreatableSelect options={item.dropdownArr} placeholder={item.label} className='react-select' classNamePrefix='select' multiple={item.multiple} onChange={(e) => props.searchHandler(e.value, item.attr)} /></Col>)
            case 'checkbox':
